...
from tensorflow.python.ops import quantized_ops
...
class QuantizedDense(module.Module):
    ...
    def __call__(self, inputs):
        ...
        if self.kernel is None:
            ...
            if last_dim is None:
                raise ValueError("The last dimension of the inputs to `QuantizedDense` should be defined.")
            # Initialize weights with glorot uniform
            limit = math_ops.sqrt(6.0 / (last_dim + self.units))
            self.kernel = variables.Variable(
                initial_value=random_ops.random_uniform([last_dim, self.units], minval=-limit, maxval=limit, dtype=inputs.dtype),
                name="kernel", trainable=True)
            if self.use_bias:
                self.bias = variables.Variable(
                    initial_value=array_ops.zeros([self.units,], dtype=inputs.dtype),
